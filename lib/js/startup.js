...
function createInvestors() {
    ...
    .then(function(response){
        ...
        for (let i = 0; i < returnedJson.results.length; i++) {
            ...
            investorArray1.push(newInvestorObj);
        }})

    let investorArray = investorArray1.splice();
    console.log(investorArray1)
    console.log(investorArray)
    populateInvestors()
}

let portalContainer, ResultsContainer
...
function populateInvestors(){
    ...
    for (let i=0; i < 6; i++) {
        ...
        let nameTextList = document.getElementsByClassName("nameText");
        nameTextList[i].innerText = `test ${i}`
